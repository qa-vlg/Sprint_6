...
class OrderPage(BasePage):
    ...
    def fill_in_user_first_name(self):
        self.enter_text(Locators.FIRST_NAME_INPUT, self.user_data['first_name'])

    @allure.step('Заполняем Фамилию пользователя')
    def fill_in_user_last_name(self):
        self.enter_text(Locators.LAST_NAME_INPUT, self.user_data['last_name'])
    
    @allure.step('Заполняем Адрес пользователя')
    def fill_in_user_address(self):
        self.enter_text(Locators.ADDRESS_INPUT, self.user_data['address'])

    @allure.step('Выбираем Станцию Метро')
    def select_subway_station(self):
        self.enter_text(Locators.SUBWAY_STATION_INPUT, self.user_data['subway_station'])
        self.click_on_element(Locators.SUBWAY_STATION_CONFIRM)

    @allure.step('Заполняем Номер Телефона пользователя')
    def fill_in_user_phone_number(self):
        self.enter_text(Locators.PHONE_NUMBER_INPUT, self.user_data['phone_number'])
    ...
    def select_delivery_date(self):
        self.enter_text(Locators.EXPECTED_DATE_INPUT, self.user_data['date'])
        self.dismiss_a_modal()

    @allure.step('Выбираем Срок Аренды')
    def select_duration(self):
        self.click_on_element(Locators.RENT_DURATION_OPEN_DROPDOWN)
        self.click_on_element(Locators.RENT_DURATION_ONE_DAY)

    @allure.step('Выбираем Цвет') 
    def pick_a_color(self):
        self.click_on_element(Locators.COLOR_PICKER_BLACK)

    @allure.step('Заполняем Комментарий к заказу')
    def enter_order_comment(self):
        self.enter_text(Locators.COMMENTS_INPUT, self.user_data['comment'])
    ...
    def confirm_order_creation(self):
        self.click_on_element(Locators.ORDER_CONFIRMATION_ACCEPT_BUTTON)
    ...
    def order_scooter(self, order_button):
        ...
        return self.get_page_element_text(Locators.ORDER_CONFIRMATION_TEXT)
